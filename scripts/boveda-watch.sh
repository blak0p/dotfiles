...
# Debounce: 30s sin cambios -> commit + push.
VAULT="$HOME/dev/Boveda"
cd "$VAULT" || exit 1

inotifywait -m -r -e close_write,moved_to --format '%w%f' "$VAULT" \
  --exclude '(.git|\.trash|\.obsidian/cache)' \
| while IFS= read -r file; do
    [[ "$file" != *.md && "$file" != *.png && "$file" != *.jpg && "$file" != *.pdf ]] && continue

    # Debounce: acumula eventos hasta 30s de silencio
    while IFS= read -t 30 -r next_file; do
    ...
    HEADING=$(git diff --cached --name-only --diff-filter=AM | head -1 | xargs head -1 2>/dev/null | sed -n 's/^# //p')
    if [ -n "$HEADING" ]; then
